...
export default function FormularioNota({ notaId = null, onSuccess, onCancel }) {
    ...
    if (notaId) {
      const cargarNota = async () => {
        try {
          ...
          const { getEstudiantesPorCurso, getEvaluacionesPorCurso } =
            await import("@/action/notas/nota");
  ...
  const handleCursoChange = async (cursoId, isEditing = false) => {
    ...
    try {
      ...
      if (estudiantesData) {
        console.log(
          `Estudiantes cargados para curso ${cursoId}:`,
      ...
      if (evaluacionesData) {
        console.log(
          `Evaluaciones cargadas para curso ${cursoId}:`,
          evaluacionesData.length
        );
        setEvaluaciones(evaluacionesData);
      }
    } catch (error) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (
        !data.estudianteId ||
        data.estudianteId === "no_seleccionado" ||
        !data.cursoId ||
        data.cursoId === "no_seleccionado" ||
        !data.evaluacionId ||
        data.evaluacionId === "no_seleccionado"
      ) {
      ...
      const { registrarNota, actualizarNota } = await import(
        "@/action/notas/nota"
      );
            ...
            <div>
              ...
              <p className="text-blue-100 text-sm mt-1">
                {notaId
                  ...
                  : "Completa el formulario para registrar una nueva calificación"}
              </p>
            </div>
          </div>
        </CardHeader>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                {form.watch("cursoId") &&
                  form.watch("cursoId") !== "no_seleccionado" && (
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-semibold text-blue-900 dark:text-blue-100">
                                  {cursoSeleccionado.areaCurricular?.nombre ||
                                    cursoSeleccionado.nombre}
                                </p>
                                <p className="text-sm text-blue-700 dark:text-blue-300">
                                  {
                                    cursoSeleccionado.nivelAcademico?.nivel
                                      ?.nombre
                                  }{" "}
                                  -{" "}
                                  {
                                    cursoSeleccionado.nivelAcademico?.grado
                                      ?.nombre
                                  }{" "}
                                  "{cursoSeleccionado.nivelAcademico?.seccion}"
                                </p>
                              </div>
                            </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select
                          disabled={
                            loadingEstudiantes || !form.watch("cursoId")
                          }
                                  ...
                                  <div className="flex items-center gap-3 py-1">
                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      {estudiante.nivelAcademico && (
                                        <span className="text-xs text-muted-foreground">
                                          {
                                            estudiante.nivelAcademico.grado
                                              ?.nombre
                                          }{" "}
                        ...
                        <FormLabel className="text-sm font-medium flex items-center gap-2">
                          ...
                          {evaluaciones.length > 0 && (
                            <Badge
                              ...
                              className="ml-auto text-xs"
                            >
                              {evaluaciones.length} disponible
                          ...
                          <div className="relative">
                            ...
                            {field.value && (
                              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                <Badge
                                  className={`${
                                    getEstadoNota(parseFloat(field.value)).color
