  ...
  switch (estado) {
    case "pendiente":
      return {
        ...
        bgGradient: "from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/10",
    ...
    case "pagado":
      return {
        ...
        bgGradient: "from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/10",
    ...
    case "vencido":
      return {
        ...
        className: "bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200 border-red-200 dark:border-red-800",
        bgGradient: "from-red-50 to-rose-50 dark:from-red-950/20 dark:to-rose-950/10",
    ...
    case "anulado":
      return {
        ...
        bgGradient: "from-gray-50 to-slate-50 dark:from-gray-950/20 dark:to-slate-950/10",
    ...
    default:
      return {
        ...
        bgGradient: "from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/10",
...
const InfoItem = ({
  ...
  const content = (
    <div
      className={`group flex items-start gap-3 p-4 rounded-xl border ${getVariantClasses()} ${highlight ? "ring-2 ring-primary/20" : ""} ${className}`}
    >
      <div className="flex-shrink-0 mt-0.5">
        <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-muted/50">{icon}</div>
      </div>
      <div className="flex-1 min-w-0">
        <dt className="text-sm font-medium text-muted-foreground mb-1">{label}</dt>
        <dd className={`text-base font-medium text-foreground break-words ${highlight ? "text-lg font-semibold" : ""}`}>
          {value}
        </dd>
      </div>
      {copyable && (
      ...
      {[...Array(2)].map((_, i) => (
        <Card key={i}>
          ...
          <CardContent className="space-y-4">
            {[...Array(4)].map((_, j) => (
              <div key={j} className="flex items-start gap-3 p-4 rounded-xl border">
...
export default function DetallePagoClient({ pago, isLoading = false }) {
  ...
  if (!pago) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center max-w-md mx-auto">
        ...
        <h3 className="text-xl font-semibold text-foreground mb-2">Pago no encontrado</h3>
        <p className="text-muted-foreground mb-6 leading-relaxed">
          No se pudo cargar la información del pago solicitado. Verifica que el enlace sea correcto.
        </p>
  ...
  const estaVencido = pago.estado === "pendiente" && new Date(pago.fechaVencimiento) < new Date()

  // Calcular días hasta vencimiento
  const diasHastaVencimiento = Math.ceil((new Date(pago.fechaVencimiento) - new Date()) / (1000 * 60 * 60 * 24))
  ...
  return (
      <div className="space-y-8 max-w-7xl mx-auto">
          ...
          <div className="relative">
            <div className="flex flex-col sm:flex-row sm:items-center gap-6">
              <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" asChild className="hover:bg-white/50 dark:hover:bg-gray-800/50">
                  <Link href="/pagos/consultar">
                    <ArrowLeft className="h-5 w-5" />
                  </Link>
                </Button>
                <div>
                  <h1 className="text-3xl font-bold text-foreground mb-1">Detalle del Pago</h1>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">{formatearFechaRelativa(pago.fechaEmision)}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center gap-2">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="icon" onClick={compartirPago} className="bg-transparent">
          ...
          <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-950/20 dark:to-rose-950/10 p-6 border border-red-200/50 dark:border-red-800/50">
            ...
            <div className="relative flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-lg font-semibold text-red-800 dark:text-red-200 mb-2">Pago Vencido</h4>
                <p className="text-red-700 dark:text-red-300 leading-relaxed">
                  Este pago venció el <strong>{formatearFecha(pago.fechaVencimiento)}</strong>.
                  {pago.mora > 0 && (
        ...
        {pago.estado === "pendiente" && !estaVencido && diasHastaVencimiento <= 7 && (
          <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/10 p-6 border border-amber-200/50 dark:border-amber-800/50">
            ...
            <div className="relative flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-lg font-semibold text-amber-800 dark:text-amber-200 mb-2">Próximo a Vencer</h4>
                <p className="text-amber-700 dark:text-amber-300 leading-relaxed">
                  Este pago vence en{" "}
                  <strong>
                    {diasHastaVencimiento} día{diasHastaVencimiento !== 1 ? "s" : ""}
                  </strong>
        ...
        <Card className={`border-0 shadow-lg bg-gradient-to-br ${estadoInfo.bgGradient}`}>
          <CardContent className="p-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className={`flex items-center justify-center w-12 h-12 rounded-xl ${estadoInfo.iconBg}`}>
                    <DollarSign className={`h-6 w-6 ${estadoInfo.iconColor}`} />
                  </div>
                  <div>
                    <h3 className="text-lg font-semibold text-foreground">Total a Pagar</h3>
                    <p className="text-sm text-muted-foreground">Monto final incluyendo ajustes</p>
                  </div>
                </div>
                <div className="space-y-2">
                  <p className="text-4xl font-bold text-foreground">
                    {montoTotal.toFixed(2)}{" "}
                    <span className="text-2xl text-muted-foreground">{pago.moneda || "USD"}</span>
                  </p>
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      {pago.descuento > 0 && (
                        <div className="flex items-center gap-1 text-green-600 dark:text-green-400">
                          ...
                          <span>
                            Descuento: {pago.descuento.toFixed(2)} {pago.moneda || "USD"}
                          </span>
                        </div>
                      )}
                      {pago.mora > 0 && (
              ...
              {pago.estado === "pendiente" && (
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button onClick={() => setMostrarFormularioPago(true)} size="lg" className="flex items-center gap-2">
          ...
          <Card className="border-0 shadow-sm bg-gradient-to-br from-blue-50/50 to-indigo-50/30 dark:from-blue-950/20 dark:to-indigo-950/10">
            ...
            <CardContent className="space-y-4">
              ...
              <InfoItem
                ...
                variant={estaVencido ? "error" : diasHastaVencimiento <= 7 ? "warning" : "default"}
                tooltip={
                  estaVencido
            ...
            <CardContent className="space-y-4">
              ...
              {pago.estado === "pendiente" && (
                <div className="space-y-3 pt-4">
                  ...
                  <p className="text-xs text-muted-foreground">Registra tu pago para actualizar el estado</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        {pago.estado === "pagado" && (
          <Card className="border-0 shadow-lg bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/10">
            ...
            <CardContent className="space-y-4">
              ...
              {pago.referenciaPago && (
                <InfoItem
                  icon={<Hash className="h-4 w-4 text-green-600 dark:text-green-400" />}
              ...
              {pago.observaciones && (
                <InfoItem
                  icon={<Info className="h-4 w-4 text-green-600 dark:text-green-400" />}
                  label="Observaciones"
              ...
              <div className="space-y-3 pt-4">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Estado del pago</span>
                  <span className="font-medium text-green-600 dark:text-green-400">Completado</span>
                </div>
              ...
              <div className="flex items-center justify-between p-6 border border-border/50 rounded-xl hover:bg-muted/30 transition-all duration-200 group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Documento adjunto • PDF</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Tooltip>
        ...
        {mostrarFormularioPago && (
          <Card className="border-0 shadow-xl bg-gradient-to-br from-background via-background to-muted/20 animate-in slide-in-from-bottom-4 duration-500">
            <CardHeader className="pb-6">
