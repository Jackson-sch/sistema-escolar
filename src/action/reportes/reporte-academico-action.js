...
export const getCalificacionesPorCurso = async (cursoId, periodoId = null) => {
  try {
    ...
    const calificacionesPorEstudiante = estudiantes.map(estudiante => {
      // Extraer directamente las notas del estudiante
      const notasEstudiante = [];
      
      // Recorrer todas las evaluaciones del curso
      curso.evaluaciones.forEach(evaluacion => {
        // Filtrar las notas que pertenecen al estudiante actual
        const notasDelEstudiante = evaluacion.notas.filter(nota => nota.estudianteId === estudiante.id);
        
        // Procesar cada nota encontrada
        notasDelEstudiante.forEach(nota => {
          // Agregar la nota con la información de la evaluación
          notasEstudiante.push({
            id: nota.id,
            valor: nota.valor,
            valorDescriptivo: nota.valorDescriptivo,
      ...
      // Agrupar notas por unidad/evaluación en lugar de por tipo fijo
      const notasPorEvaluacion = {};
      
      // Procesar directamente cada nota del estudiante
      notasEstudiante.forEach(nota => {
        if (nota.evaluacion && nota.evaluacion.nombre && nota.valor !== null) {
          // Usar el nombre de la evaluación como clave
          const nombreEvaluacion = nota.evaluacion.nombre;
          
          // Almacenar directamente el valor de la nota
          notasPorEvaluacion[nombreEvaluacion] = nota.valor.toString();
        }
      });
      
      // Mantener compatibilidad con el formato anterior pero usando los nombres reales de las evaluaciones
      let iUnidad = "N/A";
      
      // Primero intentamos encontrar una evaluación que contenga "I Unidad"
      for (const nombreEval in notasPorEvaluacion) {
        if (nombreEval.includes("I Unidad")) {
          iUnidad = notasPorEvaluacion[nombreEval];
          break;
        }
      }
      
      // Si no encontramos "I Unidad", buscamos "Examen"
      if (iUnidad === "N/A") {
        for (const nombreEval in notasPorEvaluacion) {
          if (nombreEval.toLowerCase().includes("examen")) {
            iUnidad = notasPorEvaluacion[nombreEval];
            break;
          }
        }
      ...
      return {
        ...
        calificaciones: {
          iUnidad, // Reemplazamos parcial1 por iUnidad
          // Mantenemos los otros campos para compatibilidad
          iiUnidad: notasPorEvaluacion["Examen II Unidad"] || "N/A",
          iiiUnidad: notasPorEvaluacion["Examen III Unidad"] || "N/A",
          ivUnidad: notasPorEvaluacion["Examen IV Unidad"] || "N/A",
          trabajos: calcularPromedioPorTipo(notasEstudiante, "TRABAJO"),
          examenFinal: calcularPromedioPorTipo(notasEstudiante, "EXAMEN_FINAL"),
          promedio: promedio !== null ? promedio.toFixed(1) : "N/A",
          // Incluimos todas las evaluaciones con sus nombres reales
          porEvaluacion: notasPorEvaluacion
        }
      };
    });
