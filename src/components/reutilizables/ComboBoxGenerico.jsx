...
export default function ComboboxGenerico({
  value,
  onChange,
  disabled = false,
  className,
  // Nuevos parámetros para hacer el componente genérico
  ...
  icon = null, // Icono personalizable (componente React)
  ...
  // Renderizar el elemento seleccionado
  const renderSelectedItem = () => {
    if (renderSelected && selected) {
                ...
                {filteredItems.length === 0 ? (
                  ...
                  <CommandGroup>
                    {filteredItems.map((item) => (
                      ...
                      >
                        {renderItem ? (
                          renderItem(item, value === item[valueField])
                        ) : (
                            ...
                            <div className="flex flex-col">
                              ...
                              {secondaryField && item[secondaryField] && (
                                <div className="flex gap-2 text-xs text-muted-foreground">
                                  <span className="truncate max-w-[200px]">{item[secondaryField]}</span>
                                </div>
                              )}
                            </div>
